...
const ExamControl = () => {
  ...
  let uri = `https://teachercopilot.vercel.app/theoryDutyRoaster/data/${id}/theorydutyroaster`;
  if(state === 'permanent') uri = `https://teachercopilot.vercel.app/TheoryDutyRoasterManagement/data/${id}`;
  ...
  useEffect(() => {
    if (allServiceId) {
      ...
      fetch(
        `https://teachercopilot.vercel.app/TheoryDutyRoasterManagement/data/${duty_roaster_id}`
      )
        .then((response) => response.json())
        .then((d) => {
          if (d.success) {
