...
const ExamControl = () => {
  ...
  let uri = `https://teachercopilot.vercel.app/examCommittee/data/${id}/examcommittees`;
  if(state === 'permanent') uri = `https://teachercopilot.vercel.app/TheoryExamCommitteeManagement/data/${id}`;
  ...
  useEffect(() => {
    if(allServiceId) {
      ...
      fetch(
        `https://teachercopilot.vercel.app/TheoryExamCommitteeManagement/data/${exam_routine_id}`
      )
        .then((response) => response.json())
        .then((d) => {
          if (d.success) {
